...
| #15 | Nala mock responses expansion batch 2 (8 topic baru → total 19 rule) | ✅ DONE 2026-05-04 (3 file: mock-responses.ts +8 rule, test +10 case, spec; my test file 14→24 pass; project 195→205 — 3 fail di vote-arrows.test.tsx terkait Window 2/3 modifications, di luar scope #15) | 82057ce |
...
| #17 | Visual polish — match Phase 1 fidelity (microinteractions, skeletons, footer, Suspense) | ✅ DONE 2026-05-04 (8 file baru: site-footer, 5 skeleton, animated-vote, spec; 14 file modif: globals keyframes, layout wire, 5 page Suspense + 5 card hover/scale + vote-arrows scale-bump; typecheck 0, lint 0 new, 195/195 pass) | 10da305 |
| #18 | Tier 1 BLOCKER ops setup (Sentry + Plausible + error boundary + uptime runbook) | ✅ DONE 2026-05-04 (18 file: 13 baru + 5 modif. @sentry/nextjs ~10.51.0 install, sentry.{client,server,edge}.config.ts + instrumentation.ts + lib/sentry/{client,server,edge}.ts re-exports, error.tsx + global-error.tsx brand-aligned, plausible-script.tsx + track-event.ts typed events, next.config wrapped withSentryConfig, deploy/runbooks/uptime-monitoring.md UptimeRobot setup + SLO + escalation, .env.example NEXT_PUBLIC_SENTRY_DSN + SENTRY_AUTH_TOKEN + NEXT_PUBLIC_PLAUSIBLE_DOMAIN. PII filter beforeSend (no email/username/IP). Tier 1 BLOCKER OPS cluster gap closed (1.7/10 → fix). typecheck 0, lint 0 new, 195/195 pass, build 19 routes OK. Window 3 territory clean isolation, layout.tsx surgery untuk koordinasi dengan Window 2 SiteFooter wire) | f87bd69 |
| #19 | Seed Karya body content (10 long-form artikel) — APPLIED ke production Supabase | ✅ APPLIED 2026-05-04 14:50 WIB via `pnpm tsx scripts/seed-karya-content.ts` (5 updated drafts 01-05 deterministic UUID `88888888-0001-0000-0000-00000000000{1-5}` body 3024-5168 chars; 5 skipped drafts 06-10 TITLE_MATCH karena guard idempotent — target rows udah punya body dari demo_seed). Verify query confirms 5 long-form rows present with body_len > spec target (Tulisan/Ilustrasi >1500, Vlog/Podcast >800). Code commit a73a14b sebelumnya — apply ini hanya data write, no code change. Verify helper script `scripts/verify-karya-content.ts` ditambah | a73a14b (apply) |
| #20 | Mobile responsive audit + fix 375px iPhone SE (15 page) | ✅ DONE 2026-05-04 (4 file: 2 modif minor + 2 baru. Static source analysis CLI agent — no headless browser/screenshot. Findings: Phase 2 sudah largely mobile-first dari Sprint 3 #7-#13 + #16/#17. Hanya 2 LOW-impact additive fixes: Beranda hero h1 text-5xl→text-4xl md:text-5xl lg:text-7xl + thread-list grid-cols-1 explicit. docs/MOBILE_AUDIT_2026-05-04.md: per-page acceptance + math Tebak Kata 5-col + sticky header verification + touch target gap deferred Spec #21 + hamburger menu deferred Sprint 4. 212/212 test pass. typecheck/lint pre-existing fail dari Window D a11y untracked files — bukan scope #20) | 14d604e |
| #22 | Phase 2 deploy scaffold — VPS Hostinger jubirbetaapp.spdindonesia.org | 🟡 SCAFFOLD READY 2026-05-04 (8 file baru: deploy/phase2/{README, env.production.example, nginx.conf, ecosystem.config.js, PHASE1_RENAME.md} + .github/workflows/deploy-phase2.yml + docs/DEPLOY_DECISION_2026-05-04.md ADR. Path B selected — same VPS Phase 1, parallel vhost port 3001, PM2 + Nginx reverse proxy + Let's Encrypt + GitHub Actions auto-deploy via webfactory/ssh-agent. Domain target: jubirbetaapp.spdindonesia.org (BARU); Phase 1 rename plan: jubir.spdindonesia.org → mockupjubir.spdindonesia.org (doc-only runbook, dual-listed 14 hari lalu cutover). Activation pending Mas: DNS A record + SSH ke VPS untuk setup commands + fill .env.production.local. Strict file ownership respected — zero edit ke Window 2/3/4/D batch 2 files. Pull-rebase clean.) | TBD |
...
_Last updated: 2026-05-04 by Claude Code (Spec #17 visual polish landed paralel dengan #16 + #18. Window 2 territory: site-footer dengan blue Phase 1 footer + 4-col, 5 skeleton component (thread-list, karya-grid, kelas-grid, aksi, tagih), animated-vote ui widget, hover -translate-y + shadow-jw-md di 5 card, button active:scale-[0.97] di CTA primary, vote-arrows scale-110 bump pada click. Suspense wrap di komunitas/karya/kelas/aksi/tagih pages. globals.css jw-fade-in + jw-bounce-dot keyframes. Nala mascot verified — sudah lengkap 5 ekspresi + claymorphism radial gradient + typing dot bouncing animation, no port needed.)._

_Last updated: 2026-05-04 by Claude Code (Spec #18 Tier 1 BLOCKER ops setup landed pushed f87bd69. Window 3 territory: @sentry/nextjs ~10.51.0 install, sentry.client/server/edge.config.ts + instrumentation.ts (Next.js 15 register hook + onRequestError export) + lib/sentry/{client,server,edge}.ts thin re-exports for app-code use, error.tsx (per-route brand-aligned UI) + global-error.tsx (root layout-error fallback) capture ke Sentry, plausible-script.tsx + track-event.ts typed event names, layout.tsx wire <PlausibleScript /> di <head>, next.config wrapped withSentryConfig (sourcemaps upload via SENTRY_AUTH_TOKEN, webpack.treeshake.removeDebugLogging instead of deprecated disableLogger), deploy/runbooks/uptime-monitoring.md (UptimeRobot setup Phase 1 + Phase 2 + SLO + escalation). PII filter beforeSend (no email, username, IP). Tier 1 BLOCKER OPS cluster gap closed per AUDIT_PRE_BETA_2026-05-01 (1.7/10 → fix). Layout.tsx surgery untuk koordinasi Window 2 SiteFooter wire — staged hunks separated cleanly, --only flag pada git commit untuk lock file list dari race dengan parallel agent. typecheck 0, lint 0 new, 195/195 pass, build 19 routes OK.)._

_Last updated: 2026-05-04 by Claude Code (Spec #22 Phase 2 deploy scaffold — Path B VPS Hostinger. Window E territory: deploy/phase2/ kit (README runbook + env.production.example + nginx.conf reverse-proxy + ecosystem.config.js PM2 + PHASE1_RENAME.md doc-only runbook), .github/workflows/deploy-phase2.yml (webfactory/ssh-agent reuse Phase 1 secrets pattern, build → pm2 restart → curl health check), docs/DEPLOY_DECISION_2026-05-04.md ADR (rationale: same VPS Phase 1, sudah bayar, Mas familiar, optionality preserve untuk Vercel migration kalau cost > $50/bulan post-launch). Domain pair: jubirbetaapp.spdindonesia.org (Phase 2 BARU) + mockupjubir.spdindonesia.org (Phase 1 rename per ADR migration plan, 14 hari dual-list lalu 301 redirect). Strict file ownership: 8 file baru, zero edit ke files Window 2/3/4/D currently modify. Pull-rebase autostash clean. CRITICAL stop point: scaffold ready, activation pending Mas execution (DNS provider + SSH ke VPS + fill .env.production.local with Sentry DSN, Supabase keys, Plausible domain).)._
